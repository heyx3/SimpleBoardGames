...
namespace Fitchneil.UnityLogic
{
	public class FN_InputController : BoardGames.UnityLogic.InputController<Vector2i>
	{
		protected override void Awake()
		{
			...
			OnBoardPosClicked += (boardPos) =>
			{
				var gameMode = BoardGames.UnityLogic.GameMode.GameMode<Vector2i>.Instance;
				Piece p = (Piece)gameMode.TheBoard.GetPieces(boardPos).FirstOrDefault();
				if (p != null && p.Owner.Value == gameMode.CurrentTurn.Value)
					MovesUI.Instance.CurrentPiece = p;
			};
		}
